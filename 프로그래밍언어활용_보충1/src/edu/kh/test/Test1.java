...
public class Test1 {
	...
	public static void main(String[] args) {
		...
		// 과일이름이 5개가 배열에 저장될 때 까지 반복 입력
		// 단, 중복되는 과일 존재하는 경우 추가 X
		...
		while(true) { // 무한 반복
			...
			// flag == false : 같은 과일이 존재함				 (중복 0)
			boolean flag = true;
			
			// 0번 인덱스 부터 index가 가리키는 곳 전까지
			// 입력 받은 name을 들고가서 fruits[i]과 비교
			for(int i=0 ; i < index ; i++) {
				
				// 입력 받은 name과 fruits[i]번째의 값이 같을 경우
				// == 중복되는게 존재한다!
			...
			// index가 5가 되었을 때 반복 종료
			// == 배열의 인덱스를 초과했을 때 반복 종료
			if(index == 5) {
				break; // while문 종료
			}	
		}
