...
public class PolyService {
	 ...
	 * -> 조건 : 메서드명 동일, 매개변수가 개수, 타입, 순서 하나라도 다름
	 * 
	...
	public void method1() {
		...
		// -> 자식 객체 내부에 존재하는 
		// 		부모 타입(SmartPhone)의 객체만 참조하게된다!
		
		// -> 자식 객체의 모습이 부보 객체로 변한 것 처럼 보인다!!!
...
//		s2.setAndroidVersion(15); // 오류
		...
		// The method setIosVersion(int)
		// is undefined for the type SmartPhone
...
//		int v2 = s2.getIAndroidVersion();
		...
		System.out.println("il.iosVersion : " + i1.getIosVersion());
	...
	public void method5() {
		...
		// 부모 참조 변수 = 부모 객체
		arr[0] = new SmartPhone("LCD", "KT", "엑스노스");
	...
	public void method6() {
		...
		// 다형성 적용 안된 상태
		SmartPhone smartPhone = new SmartPhone("LCD", "KT", "엑스노스");
...
//	}
	...
	public void method7() {
		...
		printToString(createPhone(3)); // new Galaxy()으로 생서된 객체 주소
	...
	public void method8() {
		...
		// System.out.println(s1 instanceof Polsyervice);
	...
	public void method9() {
		...
		for(SmartPhone s : arr) {
			...
			// s가 참조하고 있는 객체가 Galaxy
			} else if (s instanceof Galaxy) {
				((Galaxy)s).setAndroidVersion(333);
				
			} else { // Iphone동 Galaxy도 아닌 경우 == SmartPhone
				s.setDisplay("부모 화면");
			}
		}
