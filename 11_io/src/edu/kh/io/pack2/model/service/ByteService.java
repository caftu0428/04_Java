...
public class ByteService {
	...
	public void fileByteOutput()	{
		...
		try {
			...
			// -> 2byet 범주의 문자(영어,숫자,기본 특수문자 제외)는
			// 	  1byet 기반 스트림 통과 시 데이터 손실이 발생해서
			// 	  글자가 깨지는 문제 발생
			/*
			...
			*/
			...
			// 현재 시간까지의 차이를 ns(nano second 단위)론 반환
			// 1ms(밀리)     == 1/1000초
			// 1us(마이크로) == 1/1000ms
			// 1ns(나노)     == 1/1000us (1/10억 초)
			long startTime = System.nanoTime();
			
			// 출력 방법 2 : String을 byet[] 변환 후 출력
		...
		} finally { // tyr 예외 발생 여부 관계 없이 무조건 수행
	...
	public void bufferedFileByteOutput() {
		 ...
		 * 
		 * - FileOutputStream은 1바디트씩 데이터를 입출력
		 * 
		 * - BufferedOutputStream / BufferedInputStream
	...
	public void fileByetInput()	{
		...
		try {
			...
			while(true)	{ // 무한 반복
				value = fis.read(); // 1 byte씩 읽어오기
				// 단, 더 이상 읽어올 값이 없으면 -1 반환
				
				if(value == -1)break; // 다 읽어 왔으면 break;1
				
				// 읽어온 값을 StrigBuilder 객체에 추가
				sb.append((char)value); // char로 강제 형변환해서 글자 형태로 추가
			}
			
			System.out.println(sb.toString()); // 읽어온 내용 콜솔에 출력

			// InputStrieam은 flush() 없음
