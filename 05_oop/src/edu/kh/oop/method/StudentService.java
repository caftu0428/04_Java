...
0. 프로그램 종료
	...
	private void updateJava(Student s) {
		...
		System.out.print("증가 또는 감소한 Java 역략 입력(정수) : ");
		int input = sc.nextInt();
		
		// 이전 점수 + 입력 받은 점수를 전달 받은 학생에 세팅
		...
		System.out.println("Java 역락 수정 완료");
	...
	private void updateHtml(Student s) {
		...
		System.out.print("증가 또는 감소 Html 점수 : ");
		int input = sc.nextInt();
	...
	/** 매개 변수 전달 받은 두 Student의 Java 점수 비교
	 * @param s1
	 * @param s2
    	 * @return 결과 문자열
	 */
	private String compareJava(Student s1, Student s2) {
		...
		if (s1.getJava() > s2.getJava()) {
			return result + s1.getJava() + "의 점수가 더 높습니다.";
		} 
		if (s1.getJava() < s2.getJava()) {
			return result + s2.getJava() + "의 점수가 더 높습니다.";
		} 
	...
	private String compareHtml(Student s1, Student s2) {
		...
		return result + s1.getHtml() + "의 점수가 더 높습니다.";
		} 
		if (s1.getHtml() < s2.getHtml()) {
		return result + s2.getHtml() + "의 점수가 더 높습니다.";
		} 
