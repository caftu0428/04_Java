...
public class ArrayPractice {
	...
	public void practice2() {
		...
		for(int i = arr.length -1 ; i >= 0 ; i--) {
			arr[i] = arr.length - i; // 9부터 1 까지의 값 할당
...
//		int len = input.length();
	...
	public void practice9() {
		...
		int max =0; // 최대 값 저장 변수
		int min =0; // 최소 값 저장 변수
	...
	public void practice10() {
		...
		for(int i=0 ; i< randoms.length ; i++) {
			...
			// 앞쪽 인덱스에 존재하면
			// 다시 현재 인덱스에 새로운 난수를 생생해서 대입
			// (중복이 없을 때 까지 반복)
			
			// 현재 인덱스 앞쪽 까지만 순차 접근하느 for문 작성
			for(int x=0 ; x < i ; x++) {
